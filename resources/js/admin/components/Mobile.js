...
const Mobile = () => {
    ...
    const getPostData = async (page) => {
        await Axios.get(`/api/mobile?page=${page}`)
            .then(res => {
                setMobiles(res.data.mobiles.data)
                setPages(res.data.mobiles)
                setPageCount(res.data.mobiles.last_page)
                setCurrentPage(res.data.mobiles.current_page)
            })
            .catch(res => {
                console.log(res)
            })
    ...
    const handleClick = async (e, itemid, operation) => {
        ...
        if (operation === "edit") {
            ...
            history.push(`/admin/mobile/editMobile/${itemid}`)
        }

        if (operation === "view") {
            history.push(`/admin/mobile/viewMobile/${itemid}`)

        }

        if (operation === "delete") {
            await Axios.delete(`/api/mobile/${itemid}`)
    ...
    const pagination = () => {
        let pageUrls = [{ page: 1, url: `/api/mobiles?page=${1}` }]
        for (let i = 2; i <= pageCount; i++) {
            pageUrls.push({ page: i, url: `/api/mobiles?page=${i}` })
        }
    ...
    const paginateHandle = async (e, operation) => {
        ...
        if (operation === "prev") {
            ...
            if (currentPage != 1) {
                await Axios.get(`/api/mobiles?page=${pageChange}`)
                    .then(res => {
                        setMobiles(res.data.mobiles.data)
                        setPages(res.data.mobiles)
                        setPageCount(res.data.mobiles.last_page)
